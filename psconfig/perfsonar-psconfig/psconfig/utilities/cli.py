...
'''
def handle_arg_error(msg, parser, code=2, quiet=False):
    if quiet:
        sys.exit(code)
    else:
         parser.error(msg)

def print_error(msg, quiet=False):
    if not quiet:
        print(msg, file=sys.stderr)

def load_transform_config(config_file):
    ...
    if(transform_client.error):
        print_error("Error reading default transform file: {}".format(transform_client.error))
        return
...
def load_agent_config(config_file, agent_conf_client):
    ##
    #Load config file
    ...
    if agent_conf_client.error:
        print_error("Error parsing {}: {}".format(config_file, agent_conf_client.error))
        return
